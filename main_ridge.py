...
for rd in all_rounds:
    ...
    # 是否只保留最終樣本外測試期（2023–2025）
    # 若要和目前 OLS / RF / XGBoost 程式一樣保留 Val_1~Val_3 + Final_Test，設為 False。
    # 若論文第四章只比較最終樣本外期間，設為 True。
    SAVE_ONLY_FINAL = False

    # 重點：每一個 round 只能 append 一次，而且要等四個模型都跑完後再 append。
    # 原本程式在 Final_Test 裡 append 了 4 次，且部分 append 發生在 res_lstm / res_pca_lasso 尚未更新前，
    # 會造成 Ridge / Lasso 預測檔樣本數膨脹，甚至混入上一輪結果。
