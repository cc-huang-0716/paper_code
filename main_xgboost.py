...
base_feature_cols = [c for c in parq.columns if c not in exclude_list]
...
# pca
    ...
    train_pca_v = pca.fit_transform(df_pca_train[base_feature_cols])
    test_pca_v = pca.transform(df_pca_test[base_feature_cols])
    
    pc_cols = [f'PC{i+1}' for i in range(10)]
    df_pca_train_final = pd.DataFrame(train_pca_v, columns=pc_cols, index=df_pca_train.index)
...
# lstm
    ...
    print(f"執行 XGBoost 策略回測")
    
    res_pca = run_xgboost_strategy(df_pca_train_final, df_pca_test_final, pc_cols, algo_name="PCA_XGBoost")
    pca_xgb_results.append(res_pca)
    
    res_lstm = run_xgboost_strategy(df_lstm_train_final, df_lstm_test_final, lstm_cols, algo_name="LSTM_XGBoost")
...
df_pca_preds = pd.concat([res['test_data'] for res in pca_xgb_results])
df_lstm_preds = pd.concat([res['test_data'] for res in lstm_xgb_results])

df_pca_preds[['年月日', '代號', '報酬率%', 'y_pred']].to_parquet("thesis_results/predictions/PCA_xgb_preds.parquet")
df_lstm_preds[['年月日', '代號', '報酬率%', 'y_pred']].to_parquet("thesis_results/predictions/LSTM_xgb_preds.parquet")
