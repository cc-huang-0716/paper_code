...
def main():
    ...
    for rd in all_rounds:
        ...
        res_pca = run_rf_strategy(df_pca_train, df_pca_test, pc_cols, algo_name="PCA_RandomForest")
        res_lstm = run_rf_strategy(df_lstm_train, df_lstm_test, lstm_cols, algo_name="LSTM_RandomForest")
    ...
    rf_pca_preds = pd.concat([res["test_data"] for res in pca_rf_results], ignore_index=True)
    rf_lstm_preds = pd.concat([res["test_data"] for res in lstm_rf_results], ignore_index=True)
    rf_pca_preds[["年月日", "代號", "報酬率%", "y_pred"]].drop_duplicates(["年月日", "代號"], keep="last").to_parquet(f"{PRED_DIR}/PCA_rf_preds.parquet")
    rf_lstm_preds[["年月日", "代號", "報酬率%", "y_pred"]].drop_duplicates(["年月日", "代號"], keep="last").to_parquet(f"{PRED_DIR}/LSTM_rf_preds.parquet")
    print("RandomForest 任務完成")
